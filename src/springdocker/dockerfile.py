...
def build_dockerfile(options: DockerfileOptions) -> str:
    ...
    if options.platform_aware:
        lines.extend(["ARG TARGETPLATFORM", "ARG BUILDPLATFORM", ""])
    lines.extend(
        [
    ...
    if options.runtime_image == "distroless":
        ...
        lines.append(f"FROM --platform=$TARGETPLATFORM {runtime_base}")
        lines.extend(
            [
                "WORKDIR /app",
                "VOLUME /tmp",
        ...
        lines.append(f"FROM --platform=$TARGETPLATFORM eclipse-temurin:{options.java_version}-jre")
        if options.non_root:
            lines.extend(
                [
