...
def _compose_dockerfile(options: DockerfileOptions) -> DockerfileDocument:
    setup, build_cmd, jar_path = _build_setup(options.build_tool, options.recipe)
    build_step = (
    ...
    if options.recipe != "jvm-balanced":
        header_lines.append(f"# recipe: {options.recipe}")
    header_lines.append("")
    ...
    if options.recipe == "native-aot":
        build_base = f"ghcr.io/graalvm/native-image-community:{options.java_version}"
    sections.append(
        _section(
